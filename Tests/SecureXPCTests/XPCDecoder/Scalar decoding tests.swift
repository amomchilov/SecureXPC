...
final class XPCDecoder_ScalarDecodingTests: XCTestCase {
	...
	func testDecodes_XPCDouble_as_Float() throws {
		...
		try assert(xpc_double_create(Double(-Float.infinity)),
				   decodesEqualTo: -Float.infinity)
		try assert(xpc_double_create(Double(-Float.greatestFiniteMagnitude)),
				   decodesEqualTo: -Float.greatestFiniteMagnitude)
		try assert(xpc_double_create(-123.0),
				   decodesEqualTo: -123.0 as Float)
		try assert(xpc_double_create(-0.0),
				   decodesEqualTo: -0.0 as Float)
		try assert(xpc_double_create(0.0),
				   decodesEqualTo: 0.0 as Float)
		try assert(xpc_double_create(123.0),
				   decodesEqualTo: 123.0 as Float)
	...
	func testDecodes_XPCDouble_as_Double() throws {
		// "Normal" values, from lowest to highest
		try assert(xpc_double_create(-Double.infinity),
				   decodesEqualTo: -Double.infinity)
		try assert(xpc_double_create(-Double.greatestFiniteMagnitude),
				   decodesEqualTo: -Double.greatestFiniteMagnitude)
		try assert(xpc_double_create(-123.0),
				   decodesEqualTo: -123.0 as Float)
		try assert(xpc_double_create(-0.0),
				   decodesEqualTo: -0.0 as Float)
